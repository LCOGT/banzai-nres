...
pipeline {
	...
	stages {
		stage('Build image') {
			steps {
				script {
					dockerImage = docker.build("${DOCKER_IMG}", "--build-arg BANZAI_VERSION=0.9.4 .")
				}
			}
		}
		stage('Push image') {
		...
		stage('Test') {
			steps {
				script {
					sh 'docker run --rm -w=/lco/banzai-nres/ --user=root --entrypoint="pytest" ${DOCKER_IMG}" -m "not e2e" /lco/banzai-nres/'
				}
			}
		}
