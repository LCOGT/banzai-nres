...
class AssessWavelengthSolution(Stage):
    ...
    def calculate_1d_metrics(self, image, Delta_lambda):
        ...
        good_chi_squared = np.sum(
            (Delta_lambda[low_scatter_lines] / feature_centroid_uncertainty[low_scatter_lines]) ** 2) / len(
    ...
    def calculate_2d_metrics(self, image, Delta_lambda):
        ...
        bins = 20
        x_diff_Dlambda, order_diff_Dlambda = binned_statistic(x, Delta_lambda, statistic='mean', bins=bins,
                                                              range=([np.min(x)-1, np.max(x)+1])), \
                                             binned_statistic(order, Delta_lambda, statistic='mean', bins=bins,
