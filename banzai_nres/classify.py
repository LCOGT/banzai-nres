...
class StellarClassifier(Stage):
    def do_stage(self, image) -> NRESObservationFrame:
        ...
        best_metric = np.sum(cross_correlate_over_traces(image, orders_to_use, np.array([rv.value]) * rv.unit, template)['xcor'])
        ...
        for parameter, n in zip(physical_parameters, n_steps):
            ...
            for model_to_test in models_to_test:
                ...
                metric = np.sum(cross_correlate_over_traces(image, orders_to_use, np.array([rv.value]) * rv.unit, template)['xcor'])
